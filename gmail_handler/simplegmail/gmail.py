...
class Gmail(object):
    ...
    def __init__(
        ...
        try:
            ...
            self._html_handler = html2text.HTML2Text()
            self._html_handler.ignore_links = True
            self._html_handler.ignore_images = True
            self._html_handler.ignore_emphasis=True
            self._html_handler.ignore_tables=True
            self._html_handler.ignore_mailto_links = True
            self._html_handler.inline_links = False
    ...
    def add_label_to_email(self,msg_id, label_id, user_id: str='me'):
        try:
            label_modification = {'addLabelIds': [label_id]}
            message = self.service.users().messages().modify(userId=user_id,
                                                id=msg_id,
                                                body=label_modification).execute()
        except HttpError as error:
            # Pass along the error
            raise error                                        
    ...
    def _evaluate_message_payload(
        ...
        if 'attachmentId' in payload['body']:  # if it's an attachment
            ...
            html_body = data.decode('UTF-8') #BeautifulSoup(data, 'lxml', from_encoding='utf-8').body
            body = self._html_handler.handle(html_body) 
            filtered_body = body
            # pattern = r"!\[.*?\]\(\n?.*?\)"
            # filtered_body = re.sub(pattern, "", body)
            # pattern = r"-+?"
            # filtered_body = re.sub(pattern, "", filtered_body)
            # pattern = r'https?://(?:[-\w.]|(?:%[\da-fA-F]{2}))+'
            # filtered_body = re.sub(pattern, "", filtered_body)
            return [{ 'part_type': 'html', 'body': filtered_body}]
            ...
            pattern = r"!\[.*?\]\(.*?\)"
            filtered_body = re.sub(pattern, "", body)
            return [{ 'part_type': 'plain', 'body': filtered_body }]
